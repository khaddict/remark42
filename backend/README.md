...
It starts Remark42 on `127.0.0.1:8080`. To access the UI demo page go to <http://127.0.0.1:8080/web/>. Set `AUTH_GITHUB_CID`/`AUTH_GITHUB_CSEC` in the yml file to sign in with GitHub, or use the admin basic auth (`ADMIN_PASSWD`). You can tweak any of the [supported parameters](https://remark42.com/docs/configuration/parameters/) in corresponded yml file.

> 🚨 Please use `127.0.0.1` and not `localhost` to access the server; otherwise, CORS will prevent your browser from authentication to work correctly. You could alter the address with the `REMARK_URL` environment variable.
