...
> ℹ️ If you want to redefine env variables such as `PORT` on your local instance, you can add the `.env` file to the `./frontend` folder and rewrite variables as you wish. For such functional, we use `dotenv`.
...
It starts Remark42 backend on `127.0.0.1:8080`. To access the frontend running by Node, go to <http://127.0.0.1:9000/web/>. Set `AUTH_GITHUB_CID`/`AUTH_GITHUB_CSEC` in the yml file to sign in with GitHub, or use the admin basic auth (`ADMIN_PASSWD`). You can tweak any of the [supported parameters](https://remark42.com/docs/configuration/parameters/) in corresponded yml file.
